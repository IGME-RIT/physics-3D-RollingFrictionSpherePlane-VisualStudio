...
{//This function sets up the two shapes we need for this example.
	...
	for (i = 0; i < DIVISIONS; i++)
	{
		for (j = 0; j < DIVISIONS; j++)
		{
			...
			box1.vertexSet.push_back(p1.position);
	...
	box1.base.initBuffer(vertices.size(), &vertices[0]);
	...
	box1.linearVelocity = glm::vec3(3.0f, 0.0f, 0.0f);
	...
	box1.angularVelocity = glm::vec3(0.0f, 0.0f, 0.0f);
	...
	box1.inertiatensor[0][0] = 2.0f * box1.mass * radius/ 3.0f;
...
glm::vec3 getpointOfCollision(ConvexHull &A, ConvexHull &B, glm::vec3 mtv)
{
	...
	*/
	...
	for (it = A.vertexSet.begin(); it != A.vertexSet.end(); it++)
	{
		...
		{
			closestPointsA[1] = glm::vec3(transform1 * glm::vec4((*it),1.0f));			// If any other point lies close to the given point within the tolerance level, increment the total number of suspects. 
		...
		if (max < pointsA)						// If another point is found which is closer then reset the count and store the 2 closest points 
		{
			max = pointsA;
			totalSuspects1 = 1;
			pointofCollision = glm::vec3(transform1 * glm::vec4((*it), 1.0f));
	...
	for (it = B.vertexSet.begin(); it != B.vertexSet.end(); it++)
	{
		...
		{
			ClosestPointsB[1] = glm::vec3(transform2 * glm::vec4((*it), 1.0f));							// If any other point lies close to the given point within the tolerance level, increment the total number of suspects. 
		...
		if (min > pointsB)
		{
			...
			pointofCollision = glm::vec3(transform2 * glm::vec4((*it), 1.0f));
	...
	{	
		...
		l1.point1 = closestPointsA[0];
		l1.point2 = closestPointsA[1];

		l2.point1 = ClosestPointsB[0];
		l2.point2 = ClosestPointsB[1];
	...
	d = glm::dot(n, ClosestPointsB[0]);

	// This is done for even more precision, for edge to face collision, the derived point is projected on the colliding edge and that value is added to the closest edge to 
	// get the mid point of the edge touching the face of the second object.
	if (totalSuspects1 == 2 || totalSuspects2 == 2)
	{
		POC += d*n;

		d = glm::dot(startingVec1, POC) - glm::dot(startingVec1,pointofCollision);
		POC = pointofCollision + startingVec1 * d;
...
=======================================================================================================*/
		...
		// The friction from linear motion creats an impulse which acts at the point of contact creating a torque.
		// Here we calculate the angular impulse caused by the torque
		L1 = glm::cross(Ra, jf);// *jf;
		L2 = glm::cross(Rb, -jf);

		//Find the change in angular velocity caused by the impulse
		Wa2 = IA * L1;
		Wb2 = IB * L2;
		...
		B.angularVelocity += Wb2;
/*=====================================================================================================
									LINEAR FRICTION RESOLVED										( At his point All the changes in velocities caused by linear friction have been accounted for )
=======================================================================================================*/
		...
		//Add the angular velocity to the existing velocity to get the final velocities
		if (glm::dot(Wa2, A.angularVelocity) > 0.0f)
			Wa2 *= -1.0f;
		A.angularVelocity += Wa2;

		if (glm::dot(Wb2, B.angularVelocity) > 0.0f)
			Wb2 *= -1.0f;
		B.angularVelocity += Wb2;

		// Angular friction only acus on the angular motion of the object. It does not have any effect on the linear motion of the object. It merely provides resistance against the current spin of the obejct)
/*=====================================================================================================
								ANGULAR FRICTION RESOLVED								( at this point all the changes caused by the angular frictional force have been accounted for)
=======================================================================================================*/
	}
}
...
void update(float t)
{
	...
	if (returnMTV(*boxInfocus, *boxOutfocus, n, Overlap))					// Check if the collision occours
	{
		...
		resolveCollision(*boxInfocus, *boxOutfocus, POC, n);		// Resolve the velocitites of the objects (Resolution) and will account for friction between the two objects
	}
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if (key == GLFW_KEY_SPACE && (action == GLFW_PRESS))
	{
		...
		box1.linearVelocity = glm::vec3(0.0f, 0.0f, 0.0f);
		box1.angularVelocity = glm::vec3(0.0f, 0.0f, 0.0f);
		box1.RotationMat = glm::mat4(1.0f);
	}
	...
	{
		box1.linearVelocity += glm::vec3(0.0f, 1.0f, 0.0f);
	}
	...
	{
		box1.angularVelocity += glm::vec3(1.0f, 0.0f, 0.0f);
	}
	...
	{
		box1.angularVelocity += glm::vec3(1.0f, 0.0f, 1.0f);
	}
}
#pragma endregion
...
void main()
{
	...
	window = glfwCreateWindow(800, 800, "3D rolling friction", nullptr, nullptr);

	std::cout << "\n This program demonstrates implementation of angular friction \n\n\n\n\n\n\n\n\n\n";
	std::cout << " Press \"SPACE\" to reset the simulation.";
	std::cout << "\n Press \"W, A, S and D\" to increase the velcity of the box in the X and Y Directions.";
	std::cout << "\n Press \"J, K and L\" to increase the angular velcity of the box.";
